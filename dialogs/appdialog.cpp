...
void ThreadBackup::run()
{
    ...
    QByteArray ba;
    App app;
    proces->start("\"" + this->sdk + "\"" + "adb shell busybox mkdir /sdcard/DataBridge/backup");
...
//        version = this->appList.version.takeFirst();
        ...
        proces->start("\""+this->sdk+"\""+"adb shell echo -e \"app.name="+codec->toUnicode(app.appName.toUtf8())+"\" > /sdcard/DataBridge/backup/"+app.packageName+".txt");
        proces->waitForFinished(-1);
        proces->start("\""+this->sdk+"\""+"adb shell echo -e \"app.size="+app.appSize+"\" >> /sdcard/DataBridge/backup/"+app.packageName+".txt");
        proces->waitForFinished(-1);
        proces->start("\""+this->sdk+"\""+"adb shell echo -e \"app.version="+app.appVersion+"\" >> /sdcard/DataBridge/backup/"+app.packageName+".txt");
        proces->waitForFinished(-1);
        ...
        ikona.close();
        proces->start("\""+this->sdk+"\""+"adb push \""+QDir::currentPath()+"/icons/"+app.packageName+".png\" /sdcard/DataBridge/backup/");
        proces->waitForFinished(-1);
        ...
        if (this->withData)
        {
            proces->start("\""+this->sdk+"\""+"adb shell tar -zcf /sdcard/DataBridge/backup/"+app.packageName+".DATA.tar.gz /data/data/"+app.packageName);
            proces->waitForFinished(-1);
            output = proces->readAll();
            qDebug()<<"Backup app - "<<output;
        }
        if (this->withApk)
        {
            proces->start("\""+this->sdk+"\""+"adb shell cp "+app.appFile+ " /sdcard/DataBridge/backup/"+app.packageName+".apk");
            proces->waitForFinished(-1);
            output = proces->readAll();
            qDebug()<<"Backup app - "<<output;
        }
...
//        package = this->appList.package.takeFirst();
        ...
        if (this->withApk)
        {
            proces->start("\""+this->sdk+"\""+"adb shell pm install /sdcard/DataBridge/backup/" +app.packageName + ".apk");
            proces->waitForFinished(-1);
            output = proces->readAll();
            qDebug()<<"Restore pm - "<<output;
            if (output.contains("Failure [INSTALL_FAILED_INSUFFICIENT_STORAGE]"))
            {
                proces->start("\""+this->sdk+"\""+"adb shell cp /sdcard/DataBridge/backup/" +app.packageName + ".apk /data/local/tmp/");
                proces->waitForFinished(-1);
                output = proces->readAll();
            ...
            else if (output.contains("Success"), "")
            {
                if (this->withData)
                {
                    ...
                    proces->start("\""+this->sdk+"\""+"adb shell busybox tar -xzf /sdcard/DataBridge/backup/"+app.packageName+".DATA.tar.gz -C /");
                    proces->waitForFinished(-1);
                    output = proces->readAll();
